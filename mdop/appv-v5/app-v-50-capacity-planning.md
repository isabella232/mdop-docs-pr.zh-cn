...
以下建议可用作基线，以帮助确定适合你的组织的 app-v 5.0 基础结构的容量计划信息。

**重要提示** 仅将本部分中的信息用作规划 app-v 5.0 部署的常规指南。 你的系统容量要求取决于你的硬件和应用程序环境的具体详细信息。 此外，本文档中显示的性能数字是示例，你的结果可能会有所不同。
...
在设计 app-v 5.0 基础结构之前，必须确定项目的范围。 范围包括确定哪些应用程序将在哪些应用程序中可用，还包括确定目标用户及其位置。 此信息将帮助确定应实现哪种类型的 App-v 5.0 基础结构。 有关项目范围的决策必须基于你的组织的特定需求。
...
<td align="left"><p>根据要虚拟化的应用程序，可以采用不同的方式设置 app-v 5.0 基础结构。 第一个任务是定义要虚拟化的应用程序。</p></td>
</tr>
<tr class="even">
...
## 确定所需的 App-v 5.0 基础结构


**重要提示** 以下两个模型都需要在计划运行虚拟应用程序的计算机上安装 app-v 5.0 客户端。

你还可以使用电子软件分发（ESD）解决方案（如 Microsoft 系统中心配置管理器）管理你的 App-v 5.0 环境。 有关详细信息，请参阅[使用电子软件分发（ESD）部署 app-v 5.0 程序包](deploying-app-v-50-packages-by-using-electronic-software-distribution--esd-.md)。

 

-   **独立模型**-独立模型允许虚拟应用程序支持 Windows 安装程序，可在不进行流式处理的情况下分发。 独立模式下的 app-v 5.0 包括排序器和客户端;不需要其他组件。 使用名为 "排序" 的过程为虚拟化准备了应用程序。 有关详细信息，请参阅[规划 app-v 5.0 Sequencer 和客户端部署](planning-for-the-app-v-50-sequencer-and-client-deployment.md)。 建议在以下情况下使用独立模型：

    -   通过断开连接的远程用户无法连接到 app-v 5.0 基础结构。

    -   运行软件管理系统（如 Configuration Manager 2012）时。

    -   当网络带宽限制抑制电子软件分发时。

-   **完整基础结构模型**-完整的基础结构模型提供软件分发、管理和报告功能;它还包括跨网络的应用程序流。 App-v 5.0 完整基础结构模型由一个或多个 App-v 5.0 管理服务器组成。 管理服务器可用于将应用程序发布到所有客户端。 发布过程将虚拟应用程序图标和快捷方式放在目标计算机上。 它还可以将应用程序流式传输到本地用户。 有关安装管理服务器的详细信息，请参阅[规划 app-v 5.0 服务器部署](planning-for-the-app-v-50-server-deployment.md)。 建议在以下情况下使用完整的基础结构模型：

    **重要提示** App-v 5.0 完整基础结构模型要求 Microsoft SQL Server 存储配置数据。 有关详细信息，请参阅[应用 V 5.0 支持的配置](app-v-50-supported-configurations.md)。
...
以下部分提供了有关端到端应用-V 5.0 规模调整和规划的信息。 有关更多特定信息，请参阅后续部分。

**注意** 客户端上的往返行程响应时间是运行 App-v 5.0 客户端以接收来自发布服务器成功通知的计算机所用的时间。 发布服务器上的往返行程响应时间是计算机运行发布服务器以接收来自管理服务器的成功程序包元数据更新所用的时间。
...
## <a href="" id="---------app-v-5-0-management-server-capacity-planning-recommendations"></a> App-v 5.0 管理服务器容量规划建议


App-v 5.0 发布服务器需要管理服务器才能进行程序包刷新请求和程序包刷新响应。 然后，管理服务器将信息发送到管理数据库以检索信息。 有关 App-V 5.0 管理服务器支持的配置的详细信息，请参阅[app-v 5.0 支持的配置](app-v-50-supported-configurations.md)。

**注意** App-v 5.0 发布服务器上的默认刷新时间是十分钟。
...
**注意** 往返行程响应时间是计算机运行 app-v 5.0 发布服务器以接收来自管理服务器的成功程序包元数据更新所用的时间。
...
下表显示了上述每个因素的示例值。 在每个变体中，120程序包从 App-v 5.0 管理服务器刷新。
...
## <a href="" id="---------app-v-5-0-reporting-server-capacity-planning-recommendations"></a> App-v 5.0 报告服务器容量规划建议


App-v 5.0 客户端将报告数据发送到报告服务器。 然后，报告服务器将在 Microsoft SQL Server 数据库中记录信息，并将成功的通知返回到运行 App-v 5.0 客户端的计算机上。 有关应用-V 5.0 报告服务器支持的配置的详细信息，请参阅[app-v 5.0 支持的配置](app-v-50-supported-configurations.md)。

**注意** 往返行程响应时间是运行 App-v 5.0 客户端以将报告信息发送到报告服务器并从报告服务器接收成功通知所用的时间。
...
<tbody>
<tr class="odd">
<td align="left"><p>多个 App-v 5.0 客户端同时向报告服务器发送报告信息。</p></td>
<td align="left"><p></p>
<ul>
...
## <a href="" id="---------app-v-5-0-publishing-server-capacity-planning-recommendations"></a> App-v 5.0 发布服务器容量规划建议


运行 App-V 5.0 客户端的计算机连接到 app-v 5.0 发布服务器以发送发布刷新请求和接收响应。 在运行 App-v 5.0 客户端的计算机上测量往返行程响应时间。 在发布服务器上测量处理器时间。 有关应用-V 5.0 发布服务器支持的配置的详细信息，请参阅[app-v 5.0 支持的配置](app-v-50-supported-configurations.md)。

**重要提示** 下表显示了设置 App-v 5.0 发布服务器时要考虑的主要因素：

-   同时连接到单个发布服务器的客户端数。

-   每次刷新中的程序包数。

-   客户端和 app-v 5.0 发布服务器之间的环境中的可用网络带宽。
...
<tbody>
<tr class="odd">
<td align="left"><p>多个应用程序-V 5.0 客户端同时连接到一个发布服务器。</p></td>
<td align="left"><p></p>
<ul>
...
<tr class="odd">
<td align="left"><p>App-v 5.0 客户端和发布服务器之间的网络。</p>
<p></p></td>
<td align="left"><p></p>
<ul>
...
<th align="left">情形</th>
<th align="left">变体</th>
<th align="left">App-v 5.0 客户端数</th>
<th align="left">程序包数</th>
<th align="left">发布服务器上的处理器配置</th>
<th align="left">网络连接类型发布服务器/App-v 5.0 客户端</th>
<th align="left">App-v 5.0 客户端上的往返行程时间（以秒为单位）</th>
<th align="left">发布服务器上的 CPU 使用率（以% 为百分比）</th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td align="left"><p>App-v 5.0 客户端发送发布刷新请求 &amp; 接收响应，每个包含120程序包的请求</p></td>
...
## <a href="" id="---------app-v-5-0-streaming-capacity-planning-recommendations"></a> App-v 5.0 流容量规划建议


运行 App-v 5.0 客户端的计算机从流服务器流出虚拟应用程序包。 往返行程响应时间在运行 App-v 5.0 客户端的计算机上测量，并且是流式处理整个程序包所花的时间。

**重要提示** 下表列出了设置 App-v 5.0 流服务器时要考虑的主要因素：
...
<tbody>
<tr class="odd">
<td align="left"><p>多个 App-v 5.0 客户端从单个流服务器同时流处理应用程序。</p></td>
<td align="left"><p></p>
<ul>
...
<tr class="odd">
<td align="left"><p>App-v 5.0 客户端和流式服务器之间的网络。</p>
<p></p></td>
<td align="left"><p></p>
<ul>
...
<th align="left">网络连接类型流式处理服务器/App-v 5.0 客户端</th>
<th align="left">App-v 5.0 客户端上的往返行程时间（以秒为单位）</th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td align="left"><p>多个 App-v 5.0 客户端从流式处理服务器流式处理虚拟应用程序包。</p></td>
<td align="left"><p>客户端数。</p></td>
<td align="left"><p></p>
<ul>
<li><p>100</p></li>
...
<tr class="odd">
<td align="left"><p>客户端和 App-v 5.0 流服务器之间的网络连接。</p></td>
<td align="left"><p>1.5 Mbps 慢速链接网络。</p></td>
<td align="left"><p></p>
<ul>
<li><p>100</p></li>
...
每个 App-v 5.0 流服务器都应该能够处理同时流式处理的虚拟化应用程序中最少的200个客户端。
...
虽然有许多可用的容错策略和技术，但并非所有这些都适用于给定的服务。 此外，如果结合使用 app-v 5.0 角色，则某些容错选项可能不再适用，因为不兼容。
